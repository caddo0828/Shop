...
function changePage(obj) {
	//获取到键盘录入的页数
	//获取用户所按下的按键源编码48-57 ， 代表0-9,其中跳转页数不能为0
...
function changeAction(obj) {
	...
	if(hidden!=null) {
		...
		if(flag) {
			...
			if(ck) {
				 $("#form1").attr("action","http://localhost:8080/Shop/OrderCLServlet?type=shoppingCar");
				 $("#form1")[0].submit();
				 return true;
			}else {
...
function checkNums(obj,nums,bookID) {
	...
	if(nums>=1) {
		...
		if(obj.value=="immiBuy") {
			location.href="/Shop/OrderCLServlet?type=imme&id="+bookID+"&buyNums="+buyNums; 
		}else if(obj.value=="shoppingCar") {
			location.href="/Shop/BookCLServlet?type=add&id="+bookID+"&buyNums="+buyNums;
		}	
	}else {
		msgBox.innerHTML = "当前书籍库存不足，无法购买";
		msgBox.className = "error";
		return false;
	}
}
